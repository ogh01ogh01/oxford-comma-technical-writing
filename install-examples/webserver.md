...
7. Also in the `app.config.json` file, change the `baseShareUrl` property to be the server name of the Digital Workspace. For example:
   ...
   > **Note:** If Quickshare is disabled in Alfresco Content Services ignore this step.
    ...
    To disable Quickshare, set `system.quickshare.enabled=false` in the \tomcat\shared\classes\alfresco-global.properties file. For more information, see [Using the alfresco-global.properties file](https://docs.hyland.com/p/alfresco).
